...
import {TermPrimeDepositVault} from "contracts/products/termPrime/TermPrimeDepositVault.sol";
import {TermPrimeRedemptionVault} from "contracts/products/termPrime/TermPrimeRedemptionVault.sol";
 ...
 *
 * Output: deployment/<network>/termPrime-vaults.json
 */
contract DeployTermPrimeVaults is BaseProductDeployment {
    string internal constant MTOKEN_ARTIFACT = "termPrime-mtoken";
    string internal constant ARTIFACT_NAME = "termPrime-vaults";
    ...
    // --- Deployed contracts ---
    TermPrimeDepositVault internal depositVault;
    TermPrimeRedemptionVault internal redemptionVault;
    ...
    function setUp() public {
        ...
        mTokenProxy = _loadMTokenAddress("mTokenProxy", "TERM_PRIME_MTOKEN_PROXY_ADDRESS");
        mTokenDataFeedProxy = _loadMTokenAddress("mTokenDataFeedProxy", "TERM_PRIME_MTOKEN_DATA_FEED_PROXY_ADDRESS");
        ...
        sanctionsList = vm.envAddress(_envName("TERM_PRIME_SANCTIONS_LIST_ADDRESS"));
        // Fund address receives minted tokens
        tokenReceiver = vm.envAddress(_envName("TERM_PRIME_TOKEN_RECEIVER_ADDRESS"));
        feeReceiver = vm.envAddress(_envName("TERM_PRIME_FEE_RECEIVER_ADDRESS"));
        depositVaultAdmin = vm.envAddress(_envName("TERM_PRIME_DEPOSIT_VAULT_ADMIN_ADDRESS"));
        redemptionVaultAdmin = vm.envAddress(_envName("TERM_PRIME_REDEMPTION_VAULT_ADMIN_ADDRESS"));
        ...
        console.log("Token receiver (fund):", tokenReceiver);
    ...
    function _resolvePaymentTokenDataFeed() internal view returns (address) {
        (address envAddr, bool envExists) = _optionalEnvAddress("TERM_PRIME_PAYMENT_TOKEN_DATA_FEED_ADDRESS");
    ...
    function _deployVaults() internal {
        depositVaultImpl = _resolveImpl(
            address(new TermPrimeDepositVault()),
            "TERM_PRIME_DEPOSIT_VAULT_IMPL_ADDRESS",
            "termPrimeDepositVaultImpl",
            "TermPrime deposit vault"
        );

        depositVault = TermPrimeDepositVault(
            address(
                new ERC1967Proxy(
                    depositVaultImpl,
        ...
        redemptionVaultImpl = _resolveImpl(
            address(new TermPrimeRedemptionVault()),
            "TERM_PRIME_REDEMPTION_VAULT_IMPL_ADDRESS",
            "termPrimeRedemptionVaultImpl",
            "TermPrime redemption vault"
        );
        ...
        redemptionVault = TermPrimeRedemptionVault(
            address(
                new ERC1967Proxy(
                    redemptionVaultImpl,
                    abi.encodeWithSelector(
    ...
    function _grantRoles() internal {
        TermPrime mToken = TermPrime(mTokenProxy);

        midasAccessControl.grantRole(mToken.TERM_PRIME_MINT_OPERATOR_ROLE(), address(depositVault));
        midasAccessControl.grantRole(depositVault.vaultRole(), depositVaultAdmin);

        midasAccessControl.grantRole(mToken.TERM_PRIME_BURN_OPERATOR_ROLE(), address(redemptionVault));
