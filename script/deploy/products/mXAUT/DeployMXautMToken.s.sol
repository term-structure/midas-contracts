...
import {MXautCustomAggregatorFeed} from "contracts/products/mXAUT/MXautCustomAggregatorFeed.sol";
import {MXautDataFeed} from "contracts/products/mXAUT/MXautDataFeed.sol";

/**
 * @title DeployMXautMToken
 * @notice Deploys the mXAUT mToken, custom aggregator price feed,
 *         and data feeds (mToken + payment token). Saves implementation
 *         addresses to shared-implementations.json for cross-product reuse.
 *
 * Output: deployment/<network>/mXAUT-mtoken.json
 */
contract DeployMXautMToken is BaseProductDeployment {
    string internal constant ARTIFACT_NAME = "mXAUT-mtoken";
    ...
    // --- Deployed contracts ---
    mXAUT internal token;
    MXautCustomAggregatorFeed internal priceFeed;
    MXautDataFeed internal mTokenDataFeed;
    ...
    function setUp() public {
        ...
        paymentTokenPriceFeed = vm.envAddress(_envName("M_XAUT_PAYMENT_TOKEN_PRICE_FEED_ADDRESS"));
        paymentTokenSymbol = paymentToken.symbol();
        pauseOperator = vm.envAddress(_envName("M_XAUT_PAUSE_OPERATOR_ADDRESS"));
        feedAdmin = vm.envAddress(_envName("M_XAUT_FEED_ADMIN_ADDRESS"));
    ...
    function _deployMToken() internal {
        mTokenImpl = _resolveImpl(address(new mXAUT()), "M_XAUT_MTOKEN_IMPL_ADDRESS", "mXAUTMTokenImpl", "mXAUT mToken");

        token = mXAUT(
            address(new ERC1967Proxy(mTokenImpl, abi.encodeWithSignature("initialize(address)", address(midasAccessControl))))
        );
        console.log("mXAUT proxy:", address(token));
    }

    function _deployFeeds() internal {
        customAggregatorFeedImpl = _resolveImpl(
            address(new MXautCustomAggregatorFeed()),
            "M_XAUT_CUSTOM_AGGREGATOR_FEED_IMPL_ADDRESS",
            "mXAUTCustomAggregatorFeedImpl",
            "mXAUT custom aggregator feed"
        );

        priceFeed = MXautCustomAggregatorFeed(
            address(
                new ERC1967Proxy(
                    ...
                    abi.encodeWithSignature(
                        ...
                        mTokenMaxPrice,
                        mTokenMaxDeviation,
                        "mXAUT/USD Custom Aggregator Feed"
                    )
                )
            )
        );
        ...
        dataFeedImpl = _resolveImpl(
            address(new MXautDataFeed()),
            "M_XAUT_DATA_FEED_IMPL_ADDRESS",
            "mXAUTDataFeedImpl",
            "mXAUT data feed"
        );

        mTokenDataFeed = MXautDataFeed(
            address(
                new ERC1967Proxy(
                    dataFeedImpl,
                    abi.encodeWithSignature(
        ...
        paymentTokenDataFeedImpl = _resolveImpl(
            ...
            "M_XAUT_PAYMENT_TOKEN_DATA_FEED_IMPL_ADDRESS",
            "genericDataFeedImpl",
            "Generic payment token data feed"
        );

        // Payment token data feed: env override -> <SYMBOL>-datafeed.json -> deploy new
        (address envDataFeed, bool envExists) = _optionalEnvAddress("M_XAUT_PAYMENT_TOKEN_DATA_FEED_ADDRESS");
        if (envExists) {
    ...
    function _grantRoles() internal {
        midasAccessControl.grantRole(token.M_XAUT_PAUSE_OPERATOR_ROLE(), pauseOperator);
