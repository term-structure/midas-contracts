...
import {LeveragedQQQonCustomAggregatorFeed} from
    "contracts/products/leveragedQQQon/LeveragedQQQonCustomAggregatorFeed.sol";
import {LeveragedQQQonDataFeed} from "contracts/products/leveragedQQQon/LeveragedQQQonDataFeed.sol";

/**
 * @title DeployLeveragedQQQonMToken
 * @notice Deploys the LeveragedQQQon mToken, custom aggregator price feed,
 *         and data feeds (mToken + payment token). Saves implementation
 *         addresses to shared-implementations.json for cross-product reuse.
 *
 * Output: deployment/<network>/leveragedQQQon-mtoken.json
 */
contract DeployLeveragedQQQonMToken is BaseProductDeployment {
    string internal constant ARTIFACT_NAME = "leveragedQQQon-mtoken";
    ...
    // --- Deployed contracts ---
    LeveragedQQQon internal leveragedQQQon;
    LeveragedQQQonCustomAggregatorFeed internal priceFeed;
    LeveragedQQQonDataFeed internal mTokenDataFeed;
    ...
    function setUp() public {
        ...
        paymentTokenPriceFeed = vm.envAddress(_envName("LEVERAGED_QQQON_PAYMENT_TOKEN_PRICE_FEED_ADDRESS"));
        paymentTokenSymbol = paymentToken.symbol();
        pauseOperator = vm.envAddress(_envName("LEVERAGED_QQQON_PAUSE_OPERATOR_ADDRESS"));
        feedAdmin = vm.envAddress(_envName("LEVERAGED_QQQON_FEED_ADMIN_ADDRESS"));
    ...
    function _deployMToken() internal {
        mTokenImpl = _resolveImpl(
            address(new LeveragedQQQon()),
            "LEVERAGED_QQQON_MTOKEN_IMPL_ADDRESS",
            "leveragedQQQonMTokenImpl",
            "LeveragedQQQon mToken"
        );

        leveragedQQQon = LeveragedQQQon(
            address(
                new ERC1967Proxy(
        ...
        console.log("LeveragedQQQon proxy:", address(leveragedQQQon));
    }

    function _deployFeeds() internal {
        customAggregatorFeedImpl = _resolveImpl(
            address(new LeveragedQQQonCustomAggregatorFeed()),
            "LEVERAGED_QQQON_CUSTOM_AGGREGATOR_FEED_IMPL_ADDRESS",
            "leveragedQQQonCustomAggregatorFeedImpl",
            "LeveragedQQQon custom aggregator feed"
        );

        priceFeed = LeveragedQQQonCustomAggregatorFeed(
            address(
                new ERC1967Proxy(
                    ...
                    abi.encodeWithSignature(
                        ...
                        mTokenMaxPrice,
                        mTokenMaxDeviation,
                        "2xQQQon/USD Custom Aggregator Feed"
                    )
                )
            )
        );
        ...
        dataFeedImpl = _resolveImpl(
            address(new LeveragedQQQonDataFeed()),
            "LEVERAGED_QQQON_DATA_FEED_IMPL_ADDRESS",
            "leveragedQQQonDataFeedImpl",
            "LeveragedQQQon data feed"
        );

        mTokenDataFeed = LeveragedQQQonDataFeed(
            address(
                new ERC1967Proxy(
                    dataFeedImpl,
                    abi.encodeWithSignature(
        ...
        paymentTokenDataFeedImpl = _resolveImpl(
            ...
            "LEVERAGED_QQQON_PAYMENT_TOKEN_DATA_FEED_IMPL_ADDRESS",
            "genericDataFeedImpl",
            "Generic payment token data feed"
        );

        // Payment token data feed: env override → <SYMBOL>-datafeed.json → deploy new
        (address envDataFeed, bool envExists) = _optionalEnvAddress("LEVERAGED_QQQON_PAYMENT_TOKEN_DATA_FEED_ADDRESS");
        if (envExists) {
    ...
    function _grantRoles() internal {
        midasAccessControl.grantRole(leveragedQQQon.LEVERAGED_QQQON_PAUSE_OPERATOR_ROLE(), pauseOperator);
    ...
    function _saveDeployment() internal {
        ...
        vm.serializeAddress(objectKey, "mTokenProxy", address(leveragedQQQon));
