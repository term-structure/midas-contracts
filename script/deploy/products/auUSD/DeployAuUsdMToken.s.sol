...
import {AuUsdCustomAggregatorFeed} from "contracts/products/auUSD/AuUsdCustomAggregatorFeed.sol";
import {AuUsdDataFeed} from "contracts/products/auUSD/AuUsdDataFeed.sol";

/**
 * @title DeployAuUsdMToken
 * @notice Deploys the auUSD mToken, custom aggregator price feed,
 *         and data feeds (mToken + payment token). Saves implementation
 *         addresses to shared-implementations.json for cross-product reuse.
 *
 * Output: deployment/<network>/auUSD-mtoken.json
 */
contract DeployAuUsdMToken is BaseProductDeployment {
    string internal constant ARTIFACT_NAME = "auUSD-mtoken";
    ...
    // --- Deployed contracts ---
    auUSD internal token;
    AuUsdCustomAggregatorFeed internal priceFeed;
    AuUsdDataFeed internal mTokenDataFeed;
    ...
    function setUp() public {
        ...
        paymentTokenPriceFeed = vm.envAddress(_envName("AU_USD_PAYMENT_TOKEN_PRICE_FEED_ADDRESS"));
        paymentTokenSymbol = paymentToken.symbol();
        pauseOperator = vm.envAddress(_envName("AU_USD_PAUSE_OPERATOR_ADDRESS"));
        feedAdmin = vm.envAddress(_envName("AU_USD_FEED_ADMIN_ADDRESS"));
    ...
    function _deployMToken() internal {
        mTokenImpl = _resolveImpl(address(new auUSD()), "AU_USD_MTOKEN_IMPL_ADDRESS", "auUSDMTokenImpl", "auUSD mToken");

        token = auUSD(
            address(new ERC1967Proxy(mTokenImpl, abi.encodeWithSignature("initialize(address)", address(midasAccessControl))))
        );
        console.log("auUSD proxy:", address(token));
    }

    function _deployFeeds() internal {
        customAggregatorFeedImpl = _resolveImpl(
            address(new AuUsdCustomAggregatorFeed()),
            "AU_USD_CUSTOM_AGGREGATOR_FEED_IMPL_ADDRESS",
            "auUSDCustomAggregatorFeedImpl",
            "auUSD custom aggregator feed"
        );

        priceFeed = AuUsdCustomAggregatorFeed(
            address(
                new ERC1967Proxy(
                    ...
                    abi.encodeWithSignature(
                        ...
                        mTokenMaxPrice,
                        mTokenMaxDeviation,
                        "auUSD/USD Custom Aggregator Feed"
                    )
                )
            )
        );
        ...
        dataFeedImpl = _resolveImpl(
            address(new AuUsdDataFeed()),
            "AU_USD_DATA_FEED_IMPL_ADDRESS",
            "auUSDDataFeedImpl",
            "auUSD data feed"
        );

        mTokenDataFeed = AuUsdDataFeed(
            address(
                new ERC1967Proxy(
                    dataFeedImpl,
                    abi.encodeWithSignature(
        ...
        paymentTokenDataFeedImpl = _resolveImpl(
            ...
            "AU_USD_PAYMENT_TOKEN_DATA_FEED_IMPL_ADDRESS",
            "genericDataFeedImpl",
            "Generic payment token data feed"
        );

        // Payment token data feed: env override -> <SYMBOL>-datafeed.json -> deploy new
        (address envDataFeed, bool envExists) = _optionalEnvAddress("AU_USD_PAYMENT_TOKEN_DATA_FEED_ADDRESS");
        if (envExists) {
    ...
    function _grantRoles() internal {
        midasAccessControl.grantRole(token.AU_USD_PAUSE_OPERATOR_ROLE(), pauseOperator);
